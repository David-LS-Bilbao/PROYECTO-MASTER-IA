...
describe('AccountLevelCard', () => {
  it('renderiza correctamente con datos básicos', () => {
    ...
    expect(screen.getByText('Quedan 25 análisis este mes')).toBeInTheDocument();
  });

  it('muestra mensaje cuando alcanza el límite', () => {
    render(<AccountLevelCard {...mockProps} articlesAnalyzed={50} />);
    
    expect(screen.getByText('Has alcanzado el límite mensual')).toBeInTheDocument();
  });
  ...
  it('formatea correctamente la fecha de creación', () => {
    render(<AccountLevelCard {...mockProps} />);
    
    // Debe mostrar la fecha en formato español
  ...
  describe('Botón de toggle de tokens', () => {
    it('NO muestra el botón cuando no se pasa onShowTokenUsage', () => {
      render(<AccountLevelCard {...mockProps} />);
    ...
    it('llama a onShowTokenUsage al hacer click en el botón', () => {
      const onShowTokenUsage = vi.fn();
      render(
        <AccountLevelCard 
          {...mockProps} 
